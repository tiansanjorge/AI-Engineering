...
- **Solo las filas crudas de la ronda 3 sobreviven en `prompt_comparison.csv`** — cada rerun sobreescribió el archivo en vez de acumularse. Las rondas 1-2 quedan documentadas acá como números, pero ya no son auditables por separado en el CSV. `compare_prompt_techniques.py` sí acumula en un uso normal; esto pasó solo porque el archivo se borró a mano entre rondas durante esta investigación. De acá en adelante cada fila lleva una columna `round` (las filas que sobreviven se completaron como ronda 3), así que los reruns futuros se acumulan y quedan auditables por separado.
- **Los parámetros de muestreo se eligieron a criterio, no se afinaron**: `temperature=0.4` (moderado, cambia algo de ruido entre corridas por respuestas menos robóticas) y `max_completion_tokens=500` (techo de seguridad; lo observado nunca pasa de ~160 tokens). La llamada de reparación de JSON usa `temperature=0` porque debe ser determinista.
